...
const formSchema = object({
  name: string()
    ...
    .min(3, "Ім'я має бути хоча б 3 символи довжиною"),
  phone: string()
    .required("Номер телефону - обов'язкове поле")
    .min(10, "Номер має складатись принаймі з 10 цифр"),
  email: string().email(),
  details: string(),
});

type ContactTypeValues = InferType<typeof formSchema>;
const TELEGRAM_API_URL = "https://api.telegram.org/";

const Contact = () => {
  const { question, onReset, placeholder } = useContactForm();
  const form = useForm({
  ...
  const onSubmit = async (data: ContactTypeValues) => {
    const successMessage = `
      У вас є повідомлення від відвідувача ${data.name}%0A
      телефон: ${data.phone},%0A
      email: ${data.email ? data.email : "не вказаний"},%0A
      Повідомлення: %0A${data.details ? data.details : "відсутнє"}
    `;
    ...
    try {
      ...
      console.log("[CONTACT_SEND", error);
    }
  };
  ...
  return (
    <>
      <section
        id="contacts"
        className="bg-black px-8 lg:py-[120px] overflow-hidden relative"
      >
        <div className="md:container lg:mx-auto">
          <div className="flex flex-wrap -mx-4 lg:justify-between">
            <div className="w-full px-4 lg:w-1/2 xl:w-6/12">
              <div className="mb-12 max-w-[570px] lg:mb-0">
                ...
                <div className="mb-8 flex w-full max-w-[370px]">
                  <div className="mr-6 flex h-[60px] w-full max-w-[60px] items-center justify-center overflow-hidden rounded bg-gray-500 bg-opacity-5 text-gray-500 sm:h-[70px] sm:max-w-[70px]">
                    <svg
                      width={24}
                      height={24}
                      viewBox="0 0 24 24"
                ...
                <div className="mb-8 flex w-full max-w-[370px]">
                  ...
                  <div className="w-full">
                    <h4 className="mb-1 text-xl font-bold text-dark">Email</h4>
                    <p className="text-base text-body-color">
                      info@freshbrand.com
                    </p>
                  </div>
            ...
            <div className="md:w-full md:ml-0 ml-16 w-4/5 md:px-4 lg:w-1/2 xl:w-5/12">
              <div className="relative p-8 bg-black border-gray-600 border-dashed border rounded-lg shadow-lg sm:p-12">
                <form {...form} onSubmit={form.handleSubmit(onSubmit)}>
                  <div>
                    <input
                      ...
                      placeholder="Ім'я"
                      {...form.register("name")}
                      className=" mb-1 border-[f0f0f0] w-full rounded border py-3 px-[14px] text-black text-body-color outline-none focus:border-gray-500 focus-visible:shadow-none"
                    />
                  ...
                  <div>
                    <input
                      ...
                      placeholder="Email"
                      {...form.register("email")}
                      className=" mb-1 mt-5 border-[f0f0f0] w-full rounded border py-3 px-[14px] text-black text-body-color outline-none focus:border-gray-500 focus-visible:shadow-none"
                    />
                  </div>
                  <div>
                    <input
                  ...
                  <textarea
                    ...
                    className="mb-6 mt-5 border-[f0f0f0] w-full resize-none rounded border py-3 px-[14px] text-black text-body-color outline-none focus:border-gray-500 focus-visible:shadow-none"
                    defaultValue=""
                  />
                  <div>
                    <button
                      type="submit"
                      className="w-full p-3 text-white transition border rounded border-gray-500 bg-gray-500 hover:bg-opacity-90"
                    >
                      Відправити
                    </button>
                  </div>
